...
router.post("/products", async (req, res) => {
  ...
  if (add) {
    await CartManager.createCart().then((res) => {
      CartManager.addProduct(ID, JSON.parse(JSON.stringify(res.ID)));
    });
  }
});
router.get("/carts/:cid", verifyMDBID(["cid"]), async (req, res) => {
  try {
    ...
    const toSendObject = await CartManager.getProductsOfACard(cart);
    UserManager.isRegistered("cart", { toSendObject: toSendObject }, req, res);
  } catch (error) {
...
router.get("/chat", (req, res) => {
  UserManager.isRegistered("chat", {}, req, res);
});
...
router.get("/profile", (req, res) => {
  UserManager.isRegistered("profile", { user: req.session.user }, req, res);
});
...
router.get("/jwtprofile", (req, res) => {
  ...
  UserManager.isRegisteredwToken("profile", { user: myUser }, req, res);
});
